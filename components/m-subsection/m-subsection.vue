<template>
	<view class="m-subsection bg-white position-relative">
		<view class="m-subsection-menu d-flex justify-content-around align-items-center bg-white text-size-md" >
			<view class="m-subsection-menu-item padding-y-2 padding-x-2" v-for="(item, index) in sources" :key="index" @click="handleChange(index)">
				{{item.name}}
			</view>
		</view>
		<!-- 底部滑动条 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			initElementInfo () {
				getElementInfo(this, '.m-subsection-menu-item', 1).then(data => {
					console.log(data)
