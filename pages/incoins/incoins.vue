    ...
    <view class="container">
        ...
        <view class="choosemoney">
            ...
            <view class="tr" v-for="(item, index) in items" :key="index">
                <view :class="'btn ' + (val == item.money ? 'active' : '')" @tap.stop.prevent="handle" :data-id="item.id" :data-value="item.remark" :data-money="item.money">
                    {{ item.name }}
                </view>
            </view>
        </view>
        <button v-if="merid == 0" @tap="Sendincoins" class="addincoins" size="large" :disabled="disabled">模拟投币</button>
        <button v-if="merid != 0" @tap="Payincoins" class="payincoins" size="large" :disabled="disabled">确认支付</button>
        <button @tap="Backhome" class="payincoins goIndex" size="large">回到首页</button>
